...
| **SQL did not scale on either platform** | Doubling the compute knob: Confluent **0%** (autoscaler kept drawing 10 CFU), AWS **+17%** — inside the ±25% noise band, so indistinguishable from zero — for +83% cost. DataStream converts extra hardware into throughput; this SQL does not, on either cloud |
