...
| **Built twice — same logic, two APIs** | ~2,000 lines of DataStream Java and ~200 lines of Flink SQL, judged by one test suite. Both exact to the cent. **The comparison that matters is API vs API**, not cloud vs cloud: SQL was measured on both clouds and behaved the same way on each ([Confluent edition](docs/CONFLUENT_RUNBOOK.md)) |
| **DataStream API sustains 5.5× Flink SQL** | Identical live load, market and metric on the same hardware: median 5,183/s vs 950/s. The gap **widens** with key count — ~2× at 30 symbols, ~5.5× at 3,000 ([results](docs/PHASE19_RESULTS.md)) |
| **Flink SQL did not scale on either cloud** | Doubling the compute knob: Confluent **0%** (autoscaler kept drawing 10 CFU), AWS **+17%** — inside the ±25% noise band, so indistinguishable from zero — for +83% cost. DataStream converts extra hardware into throughput; this SQL does not, on either cloud |
| **Conflating before a narrow stage is worth 2.66×** | Measured at ≤30 symbols, where salting manufactured key diversity the workload lacked. **Not re-tested at 3,000 symbols**, where the keys exist naturally — the gain there may be smaller or absent |
