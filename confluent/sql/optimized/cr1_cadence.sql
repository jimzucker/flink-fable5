...
--
-- The obvious approach (throttle after a GROUP BY) does not work: a plain
-- aggregation emits an UPDATING stream and TUMBLE requires append-only input.
-- CUMULATE is the idiomatic construct for this instead — it emits a RUNNING
-- total from the start of the window once per step, which is exactly what a
-- position is. The daily window size gives a daily reset, which is correct
-- for trading anyway.
--
--   positions     : CUMULATE step 500 ms  -> <= 2 updates/key/sec
--   market values : CUMULATE step 1000 ms -> <= 1 update/key/sec
--
-- All four outputs stay in ONE fused job (the Phase 12 lesson: a chain of
-- statements costs ~15x in latency).
...
FROM (
  ...
  FROM TABLE(CUMULATE(
    TABLE (SELECT `key`, `val`, `$rowtime` FROM (
    ...
    DESCRIPTOR(`$rowtime`), INTERVAL '0.5' SECOND, INTERVAL '1' DAY))
  GROUP BY window_start, window_end,
           JSON_VALUE(`val`, '$.account'), JSON_VALUE(`val`, '$.ticker')
);
    ...
    TABLE (SELECT `key`, `val`, `$rowtime` FROM (
             SELECT `key`, `val`, `$rowtime`,
               ROW_NUMBER() OVER (PARTITION BY JSON_VALUE(`val`, '$.trade_id')
                                  ORDER BY `$rowtime` ASC) AS rn
             FROM `trades`) WHERE rn = 1),
    DESCRIPTOR(`$rowtime`), INTERVAL '1' SECOND, INTERVAL '1' DAY))
  GROUP BY window_start, window_end,
           JSON_VALUE(`val`, '$.account'), JSON_VALUE(`val`, '$.ticker')
) p
JOIN (
